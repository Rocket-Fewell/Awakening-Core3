#include "server/zone/objects/tangible/misc/PrizeToken.h"
...
void PrizeTokenImplementation::fillObjectMenuResponse(ObjectMenuResponse* menuResponse, CreatureObject* player) {
...
int PrizeTokenImplementation::handleObjectMenuSelect(CreatureObject* player, byte selectedID) {
...
int PrizeTokenImplementation::canAddObject(SceneObject* object, int containmentType, String& errorDescription) {
	SceneObject* sceno = cast <SceneObject*>(_this.getReferenceUnsafeStaticCast());
	String fullTemplate = getObjectTemplate()->getFullTemplateString();
...
int PrizeTokenImplementation::notifyObjectInserted(SceneObject* object) {
	ManagedReference<CreatureObject*> player = getParentRecursively(SceneObjectType::PLAYERCREATURE).castTo<CreatureObject*>();
...
byte PrizeTokenImplementation::checkContainerPermission(CreatureObject* player, uint16 permission) {
