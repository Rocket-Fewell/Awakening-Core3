	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		try {
			...
			if (commandType.beginsWith("createitem")) {
				...
				ManagedReference<AwakeningLogManager*> logMan = creature->getZoneServer()->getAwakeningLogManager();
				if (logMan == nullptr)
					return GENERALERROR;
				...
				bool area = false;
				float range = 64;
				String crafter = "Generated by staff member: " + creature->getFirstName() + ".";
				...
				if (area) {
					...
					String customNameTag = " (System Generated)";

					if (creature->getFirstName().toLowerCase().endsWith("elf")) {
						StringBuffer query;
						query << "SELECT santa_gift_claimed FROM account_data WHERE account_id = '" << targetGhost->getAccountID()
								<< "' AND `galaxy_id` = '" << galaxyId << "' LIMIT 1;";
						ResultSet* result = ServerDatabase::instance()->executeQuery(query);

						if (result->next()) {
							if (result->getInt(0) != 0) {
								creature->sendSystemMessage("The targeted player has already been given their gift.");
								targetCreature->sendSystemMessage("Santa has already given you your gift. Please move aside so others can get theirs.");
								return INVALIDTARGET;
							}
						} else {
							creature->sendSystemMessage("Database Error.");
							return INVALIDTARGET;
						}

						crafter = "Santa's Elves";
						customNameTag = " (Elf Generated)";
					}
					...
					StringBuffer customName;
					customName << object->getDisplayedName() << customNameTag;
					object->setCustomObjectName(customName.toString(), false);
					...
					if (inventory->transferObject(object, -1, true)) {
						...
						if (creature->getFirstName().toLowerCase().endsWith("elf")) {
							StringBuffer query1;
							query1 << "UPDATE account_data SET santa_gift_claimed = '1' WHERE account_id = '" << targetGhost->getAccountID()
